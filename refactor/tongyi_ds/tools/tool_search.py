    ...
    def _search_serper(self, query: str):
        ...
        if res.status != 200:
            return _SearchResult(
                False,
                f"[Search] Serper HTTP {res.status}: {data.decode('utf-8', 'ignore')[:300]}",
            )
        ...
        if not organic:
            return _SearchResult(
                False, f"No results found for '{query}'. Try a broader query."
            )
        ...
        for idx, page in enumerate(organic, start=1):
            ...
            snippet_text = page.get("snippet", "").replace(
